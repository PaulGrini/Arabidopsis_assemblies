...
ASSEMBLY=pilon_round3/petrea.contigs.fasta     # name of the (link to the) fasta
...
run_bowtie () {
    ...
    echo "bowtie2 --threads ${THREADS} --no-unal --no-mixed --no-discordant --sensitive --end-to-end -x ${PREFIX}_round${ROUND} -1 ../${R1} -2 ../${R2} -S ${PREFIX}_round${ROUND}.sam"
    bowtie2 --threads ${THREADS} --no-unal --no-mixed --no-discordant --sensitive --end-to-end -x ${PREFIX}_round${ROUND} -1 ../${R1} -2 ../${R2} -S ${PREFIX}_round${ROUND}.sam
